...
class SoCart extends Component
{

    public $listeners = ['productSelected', 'discountModalRefresh', 'deliveryNoteProductsSelected'];
    ...
    public function mount($cartInstance, $data = null)
    {
        ...
        if ($data) {
            ...
            foreach ($cart_items as $cart_item) {
                ...
                $this->qty_out[$cart_item->id] = $cart_item->options->qty_out ?? 0;
    ...
    public function render()
    {
        ...
        return view('livewire.so-cart', [
            'cart_items' => $cart_items
        ]);
    }

    public function deliveryNoteProductsSelected($payload)
    {
        Cart::instance($this->cart_instance)->destroy();
        $this->selectedDeliveryNoteId = $payload['purchase_id'];
        $products = $payload['products'];
     
        foreach ($products as $product) {
            $cart = Cart::instance($this->cart_instance);
            $productF = Product::where('id', $product['product_id'])->first();
            $poDetail = DeliveryNoteDetail::where('delivery_note_id', $this->selectedDeliveryNoteId)->where('product_id', $product['product_id'])->first();
            ...
            $cart->add([
                ...
                'options' => [
                    ...
                    'qty_out'                  => 0,
                    'qty_stockout'            => $poDetail->quantity,
                    'note' => ''
                ]
            ]);
    ...
    public function updateQuantity($row_id, $product_id)
    {
        ...
        if ((int)$this->quantity[$product_id] === 0) {
            if (Cart::instance($this->cart_instance)->get($row_id)) { // ✅ check existence
                Cart::instance($this->cart_instance)->remove($row_id);
            }
            return;
        }
    ...
    public function updateQuantitySo($row_id, $product_id)
    {
        $cart_item = Cart::instance($this->cart_instance)->get($row_id);
        $new_qty_out = $this->qty_out[$product_id] ?? 0;
        ...
        Cart::instance($this->cart_instance)->update($row_id, [
            'options' => [
                ...
                'product_discount'      => $cart_item->options->product_discount,
                'product_discount_type' => $cart_item->options->product_discount_type,
                'sub_total'             => $cart_item->price * $new_qty_out,
                'qty_stockout'           =>  $cart_item->options->qty_stockout,
                'qty_out'                => (int) $new_qty_out,
